...
template<>
struct hash<entt::meta_type> {
    size_t operator()(const entt::meta_type& meta_type) const;
};
...
public:
    ...
    /**
     * @brief Casts an instance to a given type. The type of the instance must be exactly the same.
     * @tparam Type Type to which to cast the instance.
     * @return A reference to the contained instance.
     */
    template<typename Type>
    ...
    /*! @copydoc fast_cast */
    template<typename Type>
    Type& fast_cast() ENTT_NOEXCEPT {
        return const_cast<Type&>(std::as_const(*this).fast_cast<Type>());
    }
...
private:
    ...
    friend std::size_t std::hash<meta_type>::operator()(const meta_type&) const;
};
...
inline size_t hash<entt::meta_type>::operator()(const entt::meta_type& meta_type) const {
    return static_cast<size_t>(reinterpret_cast<uintptr_t>(meta_type.node));
}
