...
entt::meta_any ComponentManager::construct(const entt::meta_type component_type) {
...
entt::meta_any ComponentManager::copy(const entt::meta_handle component) {
...
entt::meta_any ComponentManager::move(const entt::meta_handle component) {
    assert(is_registered(component.type()));
    assert(is_move_constructible(component.type()));
    return descriptors.find(component.type())->second.move(component);
}

entt::meta_any ComponentManager::move_or_copy(const entt::meta_handle component) {
...
const char* ComponentManager::get_name(const entt::meta_type component_type, const char* const fallback) {
    assert(is_registered(component_type));
...
bool ComponentManager::is_registered(const entt::meta_type component_type) {
...
bool ComponentManager::is_ignored(const entt::meta_type component_type) {
    assert(is_registered(component_type));
