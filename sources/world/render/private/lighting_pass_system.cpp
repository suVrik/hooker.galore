...
#include "shaders/lighting_pass_point/lighting_pass_point.fragment.h"
#include "shaders/lighting_pass_point/lighting_pass_point.vertex.h"
...
static const bgfx::EmbeddedShader LIGHTING_PASS_POINT_SHADER[] = {
        BGFX_EMBEDDED_SHADER(lighting_pass_point_vertex),
        BGFX_EMBEDDED_SHADER(lighting_pass_point_fragment),
        BGFX_EMBEDDED_SHADER_END()
};
...
LightingPassSystem::LightingPassSystem(World& world)
    ...
    bgfx::ShaderHandle point_vertex_shader_handle = bgfx::createEmbeddedShader(LIGHTING_PASS_POINT_SHADER, type,
                                                                               "lighting_pass_point_vertex");
    bgfx::ShaderHandle point_fragment_shader_handle = bgfx::createEmbeddedShader(LIGHTING_PASS_POINT_SHADER, type,
                                                                                 "lighting_pass_point_fragment");
    lighting_pass_single_component.lighting_pass_point_program = bgfx::createProgram(point_vertex_shader_handle,
                                                                                     point_fragment_shader_handle,
                                                                                     true);
    ...
    lighting_pass_single_component.dir_light_position_uniform  = bgfx::createUniform("u_dir_light_position",  bgfx::UniformType::Vec4);
    lighting_pass_single_component.dir_light_view_proj_uniform = bgfx::createUniform("u_dir_light_view_proj", bgfx::UniformType::Mat4);
    ...
    lighting_pass_single_component.shadow_map_texture_uniform  = bgfx::createUniform("s_shadow_map",          bgfx::UniformType::Sampler);
...
LightingPassSystem::~LightingPassSystem() {
    ...
    destroy_valid(lighting_pass_single_component.dir_light_position_uniform);
    destroy_valid(lighting_pass_single_component.dir_light_view_proj_uniform);
    ...
    destroy_valid(lighting_pass_single_component.lighting_pass_point_program);
    destroy_valid(lighting_pass_single_component.normal_metal_ao_uniform);

    destroy_valid(lighting_pass_single_component.shadow_map_texture_uniform);
...
void LightingPassSystem::update(float /*elapsed_time*/) {
    ...
    bgfx::setTexture(6, lighting_pass_single_component.shadow_map_texture_uniform, shadow_pass_single_component.shadow_map_texture);

    // TODO proper directional light position
    ...
    bgfx::setUniform(lighting_pass_single_component.dir_light_position_uniform, &light_model);
    bgfx::setUniform(lighting_pass_single_component.dir_light_view_proj_uniform, &light_view_projection);
    ...
    bgfx::setViewTransform(LIGHTING_PASS, glm::value_ptr(camera_single_component.view_matrix),
                           glm::value_ptr(camera_single_component.projection_matrix));

    bgfx::setVertexBuffer(0, quad_single_component.vertex_buffer, 0, QuadSingleComponent::NUM_VERTICES);
    bgfx::setIndexBuffer(quad_single_component.index_buffer, 0, QuadSingleComponent::NUM_INDICES);

    bgfx::setTexture(0, lighting_pass_single_component.color_roughness_uniform,
                     geometry_pass_single_component.color_roughness_texture);
    bgfx::setTexture(1, lighting_pass_single_component.normal_metal_ao_uniform,
                     geometry_pass_single_component.normal_metal_ao_texture);
    bgfx::setTexture(2, lighting_pass_single_component.depth_uniform, geometry_pass_single_component.depth_texture);

    world.view<LightComponent, TransformComponent>().each(
            [&](entt::entity, LightComponent& light_component, TransformComponent& transform_component) {
                const glm::vec4 light_position(transform_component.translation, 0.f);
                const glm::vec4 light_color(light_component.color, 0.f);
                bgfx::setUniform(lighting_pass_single_component.light_position_uniform, glm::value_ptr(light_position),
                                 1);
                bgfx::setUniform(lighting_pass_single_component.light_color_uniform, glm::value_ptr(light_color), 1);
                dd::sphere(transform_component.translation, glm::vec3(1.f, 1.f, 1.f), 0.1f);
            });
    ...
    bgfx::setStencil(BGFX_STENCIL_TEST_EQUAL | BGFX_STENCIL_FUNC_REF(1) | BGFX_STENCIL_FUNC_RMASK(1) |
                     ...
                     BGFX_STENCIL_NONE);
    bgfx::setState(BGFX_STATE_WRITE_RGB | BGFX_STATE_WRITE_A | BGFX_STATE_CULL_CW |
                   BGFX_STATE_BLEND_FUNC_SEPARATE(BGFX_STATE_BLEND_ONE, BGFX_STATE_BLEND_ONE, BGFX_STATE_BLEND_ZERO,
                                                  BGFX_STATE_BLEND_ONE));

    bgfx::submit(LIGHTING_PASS, lighting_pass_single_component.lighting_pass_point_program);
}
