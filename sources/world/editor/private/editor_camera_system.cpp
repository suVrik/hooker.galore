...
namespace editor_camera_system_details {
...
void EditorCameraSystem::update(float elapsed_time) {
    ...
    if (world.valid(camera_single_component.active_camera) && world.has<EditorCameraComponent, TransformComponent>(camera_single_component.active_camera)) {
        ...
        float speed = editor_camera_system_details::CAMERA_SPEED * elapsed_time;
        if (normal_input_single_component.is_down(Control::KEY_SHIFT)) {
            speed *= editor_camera_system_details::CAMERA_SPEED_INCREASE_FACTOR;
        }
        if (normal_input_single_component.get_mouse_wheel() != 0) {
            speed *= std::abs(normal_input_single_component.get_mouse_wheel()) * editor_camera_system_details::CAMERA_SPEED_WHEEL_FACTOR;
        }
        ...
        if (!selected_entity_single_component.selected_entities.empty() && (attached_mode || normal_input_single_component.get_mouse_wheel() != 0)) {
            update_attached_camera(camera_single_component, normal_input_single_component, selected_entity_single_component, editor_camera_component, transform_component, speed);
        } else {
...
void EditorCameraSystem::update_attached_camera(CameraSingleComponent& camera_single_component, NormalInputSingleComponent& normal_input_single_component,
                                                SelectedEntitySingleComponent& selected_entity_single_component, EditorCameraComponent& editor_camera_component,
                                                TransformComponent& transform_component, const float speed) const noexcept {
    ...
    if (distance_to_object > glm::epsilon<float>()) {
        ...
        if (is_dragging_camera || normal_input_single_component.is_down(Control::BUTTON_MIDDLE)) {
            ...
            editor_camera_component.yaw += normal_input_single_component.get_delta_mouse_x() * editor_camera_system_details::MOUSE_SENSITIVITY;
            editor_camera_component.pitch += normal_input_single_component.get_delta_mouse_y() * editor_camera_system_details::MOUSE_SENSITIVITY;
...
void EditorCameraSystem::update_free_camera(CameraSingleComponent& camera_single_component, NormalInputSingleComponent& normal_input_single_component,
                                            EditorCameraComponent& editor_camera_component, TransformComponent& transform_component, const float speed) const noexcept {
    float delta_x = 0.f;
